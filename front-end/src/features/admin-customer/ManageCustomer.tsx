...
export function ManageCustomer() {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter((customer) => {
      const matchSearch = customer.fullName
        .toLowerCase()
  ...
  const handleDelete = (customer: Customer) => {
    ...
    deleteCustomer.mutate(customer.userId, {
      onError(error) {
        setListError(getErrorMessage(error, "Xóa tài khoản thất bại."));
      },
    });
  };

  const handleSubmit = (
    values: CreateCustomerPayload | UpdateCustomerPayload,
  ) => {
    setFormError(null);
    ...
    createCustomer.mutate(values as CreateCustomerPayload, {
      onSuccess() {
        setDialogOpen(false);
      },
      onError(error) {
        setFormError(getErrorMessage(error, "Tạo khách hàng thất bại."));
      },
    });
  };
  ...
  return (
    <>
      ...
      />
      {listError && <p className="mt-4 text-sm text-status-danger">{listError}</p>}
      <CustomersTable
